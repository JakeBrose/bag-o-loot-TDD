    ...
    db
      ...
      .run(
        ...
        err => {
          ...
          insertChildRows().then(child_ids => {
            return insertToyRows()
            // resolve({ children: child_ids, toys: insertToyRows() });
          })
          .then( (toysIds) => {
            resolve(toysIds)
          })
...
const insertChildRows = () => {
  return Promise.all(
    children.map(({ first_name, last_name, isGood }) => {
      return new Promise((resolve, reject) => {
        db.run(
...
const insertToyRows = () => {
  return Promise.all(
    toys.map(({ name, delivered, child_id }) => {
      return new Promise((resolve, reject) => {
        db.run(
